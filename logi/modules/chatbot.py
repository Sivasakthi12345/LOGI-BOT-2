...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            ...
            return
        await lel.edit(
            f"logi AI Actived by {message.from_user.mention()} for users in {message.chat.title}"
        )
        ...
        if not Escobar:
            ...
            return
        await lel.edit(
            f"logi AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        test = test.replace(
            "I live in @logi_channel.", "I live in San Francisco, California."
        )
        ...
        response = response.replace(
            "Have the control right.", "@cl_me_logesh is my owner."
        )
        response = response.replace(
            "Hi, my friend, what can I do for you today?",
            "Hi, My name is logi Nice to meet you",
        ...
        response = response.replace(
            "I live in San Francisco, California.", "I live in @ekiko_channel."
        )
        response = response.replace("I made myself", "I was Created by @cl_me_logesh")
        response = response.replace(
            "Hi, my friend! Do you want me to tell you a joke?",
...
@pbot.on_message(
    filters.regex("logi|logi|robot|logi|sena")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
